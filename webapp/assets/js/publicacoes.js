...
function atualizarPublicacao(evento) {
  ...
  const publicaoId = $(this).data('publicacao-id')
  
  $.ajax({
    url: `http://localhost:3000/publicacoes/${publicaoId}`,
    method: "PUT",
    data: {
      titulo: $('#titulo').val(),
      conteudo: $('#conteudo').val()
...
function deletarPublicacao(evento){
  ...
  Swal.fire({
    title: "Atenção!",
    text: "Tem certeza que deseja excluir essa publicação? Essa ação é irrevelsível",
