...
func AtualizarSenha(w http.ResponseWriter, r *http.Request) {
	...
	if usuarioIdToken != usuarioId {
		respostas.Erro(w, http.StatusForbidden, errors.New("Não é possível atualizar a senha de um usuário não seja o seu"))
	}
	...
	defer db.Close()
	repositorio := repositorios.NovoRepositoriosDeUsuarios(db)
	senhaSalvaDB, erro := repositorio.BuscarSenha(usuarioId)
	...
	if erro = seguranca.VerificarSenha(senhaSalvaDB, senha.Atual); erro != nil {
		respostas.Erro(w, http.StatusUnauthorized, errors.New("A senha atual não condiz com a senha salva no DB"))
		return
	}
